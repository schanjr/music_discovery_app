...
const ArtistAlbums: React.FC = () => {
  ...
  const albumDetails = useSelector((state: RootState) => state.albumDetails);
  ...
  const handleRowClick = async (albumName: string, artistName: string) => {
    ...
    const albumKey = toAlbumDetailsSliceKey(encodedArtistName, encodedAlbumName);
    try {
      if (!albumDetails[albumKey]) {
        const lastFM = new LastFM()
        const albumDetails = await lastFM.getAlbumDetails(artistName, albumName)
        dispatch(setAlbumDetails({[albumKey]: albumDetails}));
      }
              ...
              <Table className={classes.table}>
                ...
                <TableBody>
                  {sortedAlbums.map((album: Album) => (
                          <TableRow key={album.name} className={classes.tableRowHover}
                                    onClick={() => handleRowClick(album.name, album.artist)}>
